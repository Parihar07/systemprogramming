...
impl Restaurant{
    fn chef_special(&self) -> Option<Food> {
        ...
        if(self.reservations<12){
            return Option::Some(Food{
                name:String::from("Unin Sishu"),
            })
        } else {
            return Option::Some(Food{
                name:String::from("Strip Steak Sishu"),
            })
        }
    }
    ...
    fn deliver_burger(&self, address:&str) ->Result<Food,String> {
        if self.has_mice_infestation {
            return Result::Err(String::from("Sorry we have mice infestation"));
        }
...
fn main() {
    println!("Hello, understanifn this options and other add hoc functionalities of Rust!");
    let zing1 = Restaurant{
        reservations:11,
        has_mice_infestation:true,
    };

    println!("{:?}", zing1.chef_special());
    println!("{:?}", zing1.deliver_burger("123 Elm Street"));


     let zing2 = Restaurant{
        reservations:15,
        has_mice_infestation:false,
    };

    println!("{:?}", zing2.chef_special());
    println!("{:?}", zing2.deliver_burger(""));
    println!("{:?}", zing2.deliver_burger("123 Main Street"));
}
